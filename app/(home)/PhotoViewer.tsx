...
const EnhancedPhoto: React.FC<{
  ...
  currentIndex: number;
  onMediaPress: () => void;
}> = ({ uri, index, currentIndex, onMediaPress }) => {
...
const PhotoViewer = () => {
  ...
  const getMediaItems = useCallback((): MediaItem[] => {
    ...
    try {
      ...
      devLog('GENERAL', 'PhotoViewer: Error processing media items:', error);
      return [];
    }
  }, [selectedItem]);
  ...
  const { currentIndex, setCurrentIndex, animationValues } = useMediaViewerState(
    true, // always visible when component is mounted
    0, // initial index
    () => {
  ...
  useFocusEffect(
    React.useCallback(() => {
      devLog('GENERAL', 'PHOTO_VIEWER: Screen focused - setting status bar to light content');
      RNStatusBar.setBarStyle('light-content', true);
      RNStatusBar.setHidden(false, 'fade');
      
      return () => {
        devLog('GENERAL', 'PHOTO_VIEWER: Screen unfocused - restoring status bar');
  ...
  // Handle deep link with postId parameter
  useEffect(() => {
    ...
    if (postId && (!selectedItem || selectedItem._id !== postId)) {
      devLog('GENERAL', 'PhotoViewer: Loading data for postId:', postId);
        ...
        try {
          ...
          if (response.data && (response.data.post || response.data._id)) {
            ...
            setTimeout(() => {
              ...
              devLog('GENERAL', 'PhotoViewer: Successfully loaded photo data');
            }, 100);
          } else {
            devLog('GENERAL', 'PhotoViewer: Post not found');
            router.replace('/(tabs)');
          }
        } catch (error) {
          devLog('GENERAL', 'PhotoViewer: Error fetching post data:', error);
          router.replace('/(tabs)');
        } finally {
  ...
  useEffect(() => {
    const backAction = () => {
      devLog('GENERAL', 'PhotoViewer: Hardware back button pressed');
      
      // Check if any bottom sheet is open first
      if (activeSheet) {
        devLog('GENERAL', 'PhotoViewer: Closing bottom sheet first');
        setActiveSheet(null);
        return true;
      }
      ...
      if (userProfileUserId) {
        devLog('GENERAL', 'PhotoViewer: Closing user profile sheet first');
        handleUserProfileClose();
        return true;
      }
      
      // Otherwise, close the PhotoViewer
      devLog('GENERAL', 'PhotoViewer: Closing PhotoViewer');
      handleBack();
    ...
    return () => {
      devLog('GENERAL', 'PhotoViewer: Removing back handler');
      clearTimer();
      backHandler.remove();
    };
  }, [activeSheet, userProfileUserId, clearTimer]);
