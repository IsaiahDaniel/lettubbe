...
export default function CommunityDeepLink() {
    ...
    const checkNavigationReady = () => {
      try {
        ...
        if (router && typeof router.replace === 'function') {
          console.log('🏘️ COMMUNITY_DEEPLINK: Navigation stack is ready');
          setNavigationReady(true);
          return true;
        }
      } catch (error) {
        console.log('🏘️ COMMUNITY_DEEPLINK: Navigation not ready yet:', error);
      }
      return false;
    };
    ...
    const interval = setInterval(() => {
      ...
      if (checkNavigationReady() || attempts >= maxAttempts) {
        ...
        if (attempts >= maxAttempts) {
          console.log('🏘️ COMMUNITY_DEEPLINK: Navigation readiness timeout, proceeding anyway');
          setNavigationReady(true);
        }
      }
    }, 100);
  ...
  useEffect(() => {
    ...
    if (!id) {
      console.log('🏘️ COMMUNITY_DEEPLINK: No ID provided, redirecting to home');
      router.replace('/(tabs)');
      return;
    }
    
    console.log('🏘️ COMMUNITY_DEEPLINK: Navigation ready, redirecting to Community with id:', id);
    // Direct redirect to Community - it handles its own loading state
